defmodule GreenhouseTycoon.SetTargetLight.TargetLightSetToGreenhouseCacheV1 do
  @moduledoc """
  Subscriber that listens to target light set PubSub events and updates the cache.

  This subscriber follows the vertical slicing architecture and focuses on updating the target light in the cache.
  ...
  def init(_opts) do
    Logger.info("🗄️  Starting target light set cache subscriber")
  ...
  # Handle target light set
  def handle_info({:target_light_set, partial_read_model}, state) do
    update_cache_field(partial_read_model, :target_light_set, fn read_model ->
      %{read_model |
        target_light: partial_read_model.target_light,
  ...
  # Update cache field with new target light
