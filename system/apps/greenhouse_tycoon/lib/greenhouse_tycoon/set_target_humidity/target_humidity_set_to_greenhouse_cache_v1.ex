defmodule GreenhouseTycoon.SetTargetHumidity.TargetHumiditySetToGreenhouseCacheV1 do
  @moduledoc """
  Subscriber that listens to target humidity set PubSub events and updates the cache.

  This subscriber follows the vertical slicing architecture and focuses on updating the target humidity in the cache.
  ...
  def init(_opts) do
    Logger.info("🗄️  Starting target humidity set cache subscriber")
  ...
  # Handle target humidity set
  def handle_info({:target_humidity_set, partial_read_model}, state) do
    update_cache_field(partial_read_model, :target_humidity_set, fn read_model ->
      %{read_model |
        target_humidity: partial_read_model.target_humidity,
  ...
  # Update cache field with new target humidity
