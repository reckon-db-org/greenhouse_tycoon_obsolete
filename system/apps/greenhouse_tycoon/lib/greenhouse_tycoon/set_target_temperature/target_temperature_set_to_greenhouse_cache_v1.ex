defmodule GreenhouseTycoon.SetTargetTemperature.TargetTemperatureSetToGreenhouseCacheV1 do
  @moduledoc """
  Subscriber that listens to target temperature set PubSub events and updates the cache.

  This subscriber follows the vertical slicing architecture by living in the same slice
  as the events it processes. It focuses on updating the target temperature in the cache.
  ...
  def init(_opts) do
    Logger.info("🗄️  Starting target temperature set cache subscriber")
  ...
  # Handle target temperature set
  def handle_info({:target_temperature_set, partial_read_model}, state) do
    update_cache_field(partial_read_model, :target_temperature_set, fn read_model ->
      %{read_model |
        target_temperature: partial_read_model.target_temperature,
  ...
  # Update cache field with new target temperature
