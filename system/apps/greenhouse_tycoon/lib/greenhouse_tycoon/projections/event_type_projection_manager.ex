defmodule GreenhouseTycoon.Projections.EventTypeProjectionManager do
  ...
  def init(_opts) do
    ...
    children =
      Enum.map(@event_handlers, fn {event_type, handler_module} ->
        %{
    ...
    case Supervisor.terminate_child(__MODULE__, projection_id) do
      :ok ->
        case Supervisor.restart_child(__MODULE__, projection_id) do
          ...
          {:error, reason} ->
            Logger.error(
              "EventTypeProjectionManager: Failed to restart #{event_type} projection: #{inspect(reason)}"
            )
      ...
      {:error, reason} ->
        Logger.error(
          "EventTypeProjectionManager: Failed to terminate #{event_type} projection: #{inspect(reason)}"
        )
