defmodule GreenhouseTycoon.Projections.EventTypeProjection do
  ...
  def init(opts) do
    ...
    case start_circuit_breaker(circuit_breaker_name) do
      ...
      {:error, reason} ->
        Logger.error(
          "EventTypeProjection[#{event_type}]: Failed to start circuit breaker: #{inspect(reason)}"
        )
    ...
    case create_subscription(state) do
      ...
      {:error, reason} ->
        Logger.error(
          "EventTypeProjection[#{event_type}]: Failed to create subscription: #{inspect(reason)}"
        )
  ...
  def handle_info(msg, state) do
    ...
    Logger.warning(
      "EventTypeProjection[#{state.event_type}]: Received unexpected message: #{inspect(msg)}"
    )
  ...
  defp get_store_id do
    ...
    Keyword.get(commanded_config, :store_id, :greenhouse_tycoon)
  end
  ...
  defp create_subscription(state) do
    ...
    case API.save_subscription(
           ...
           0,
           # Use proxy process as subscriber
           proxy_pid
         ) do
      :ok ->
        Logger.info(
          "EventTypeProjection[#{state.event_type}]: Subscribed to event type #{state.event_type} via proxy #{inspect(proxy_pid)}"
        )
      ...
      {:error, reason} ->
        ...
        Logger.error(
          "EventTypeProjection[#{state.event_type}]: Failed to subscribe to event type #{state.event_type}: #{inspect(reason)}"
        )
    ...
    case EventDeduplicator.process_with_dedup(event_id, fn ->
           ...
           case CircuitBreaker.call(state.circuit_breaker_name, fn ->
                  ...
                  if greenhouse_id do
                    ...
                    Logger.warning(
                      "EventTypeProjection[#{state.event_type}]: Could not extract greenhouse_id from stream_id: #{event.event_stream_id}"
                    )
             ...
             {:error, {:circuit_breaker, reason}} ->
               Logger.error(
                 "EventTypeProjection[#{state.event_type}]: Circuit breaker prevented processing: #{inspect(reason)}"
               )
             ...
             {:error, :circuit_breaker_open} ->
               Logger.warning(
                 "EventTypeProjection[#{state.event_type}]: Circuit breaker is open, skipping event"
               )
  ...
  defp extract_greenhouse_id(stream_id) do
    ...
    case String.split(stream_id, "_", parts: 3) do
      ["greenhouse", "tycoon", greenhouse_id] when greenhouse_id != "" ->
        greenhouse_id
  ...
  defp subscription_proxy_loop(metadata) do
    receive do
      {:events, [%ExESDB.Schema.EventRecord{} = event_record]} ->
        ...
        Logger.info(
          "SubscriptionProxy[#{event_type}]: Received EventRecord #{event_record.event_type} for stream #{event_record.event_stream_id}"
        )

        # Only forward events that match our event type
        if event_record.event_type == event_type do
          Logger.info(
            "SubscriptionProxy[#{event_type}]: Forwarding matching event to subscriber #{inspect(subscriber)}"
          )

          send(subscriber, {:events, [event_record]})
        else
          Logger.debug(
            "SubscriptionProxy[#{event_type}]: Ignoring non-matching event type #{event_record.event_type}"
          )
        end
      ...
      {:events, events} when is_list(events) ->
        ...
        if length(matching_events) > 0 do
          Logger.info(
            "SubscriptionProxy[#{event_type}]: Forwarding #{length(matching_events)} matching events"
          )
      ...
      message ->
        ...
        Logger.warning(
          "SubscriptionProxy[#{event_type}]: Received unknown message: #{inspect(message)}"
        )
