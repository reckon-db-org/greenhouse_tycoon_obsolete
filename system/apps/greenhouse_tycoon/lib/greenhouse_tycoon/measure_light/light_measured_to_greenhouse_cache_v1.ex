defmodule GreenhouseTycoon.MeasureLight.LightMeasuredToGreenhouseCacheV1 do
  @moduledoc """
  Subscriber that listens to light measurement PubSub events and updates the cache.

  This subscriber follows the vertical slicing architecture and focuses on updating the light measurement in the cache.
  ...
  def init(_opts) do
    Logger.info("🗄️  Starting light measurement cache subscriber")
  ...
  # Handle light measurements
  def handle_info({:light_measured, partial_read_model}, state) do
    update_cache_field(partial_read_model, :light_measured, fn read_model ->
      %{read_model |
        current_light: partial_read_model.current_light,
  ...
  # Update cache field with new light
