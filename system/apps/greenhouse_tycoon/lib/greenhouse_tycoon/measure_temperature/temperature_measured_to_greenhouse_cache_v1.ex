defmodule GreenhouseTycoon.MeasureTemperature.TemperatureMeasuredToGreenhouseCacheV1 do
  @moduledoc """
  Subscriber that listens to temperature measurement PubSub events and updates the cache.

  This subscriber follows the vertical slicing architecture by living in the same slice
  as the events it processes. It focuses on updating the temperature measurement in the cache.
  ...
  def init(_opts) do
    Logger.info("🗄️  Starting temperature measurement cache subscriber")
  ...
  def handle_info({:temperature_measured, partial_read_model}, state) do
    update_cache_field(partial_read_model, :temperature_measured, fn read_model ->
      %{read_model |
        current_temperature: partial_read_model.current_temperature,
  ...
  # Update cache field with new temperature
